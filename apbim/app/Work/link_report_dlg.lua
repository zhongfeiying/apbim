...
local analysis_ = iup.button{title="Excel",rastersize="60X"};
...
local function init_list()
	...
	s = require'sys.table'.filter(s,function(k,v) if type(curs[v.dstid])=='table' then return true else return false end end)
