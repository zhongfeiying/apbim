...
local function get_id()
	local id = tree_:get_index_id(0,'__title','family') 
	return id
end
...
local function project_attr(arg)
	return {
		title = arg.name;
		data = {
			rmenu = require 'app.projectmgr.workspace.family.rmenu'.get_project_menu;
			id = arg.id;
		};
		kind = 'branch';
...
local function branch_attr(arg)
	return {
		title = arg.name;
		data = {
			rmenu = require 'app.projectmgr.workspace.family.rmenu'.get_branch_menu;
			id = arg.id;
		};
		kind = 'branch';
...
local function leaf_attr(arg)
	return {
		title = arg.name;
		data = {
			rmenu = require 'app.projectmgr.workspace.family.rmenu'.get_leaf_menu;
			id = arg.id;
		};
		kind = 'leaf';
