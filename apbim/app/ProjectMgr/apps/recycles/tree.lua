...
function init_root()
	root_id_ = tree_workspace_.add_root{
		...
		data= {
			rmenu = require 'app.projectmgr.apps.recycles.rmenu'.get;
		};
	};
end
...
local function project_attr(arg)
	return {
		title = arg.name;
		data = {
			rmenu = require 'app.projectmgr.apps.recycles.rmenu'.get_project_menu;
			id = arg.id;
		};
		kind = 'branch';
...
local function branch_attr(arg)
	return {
		title = arg.name;
		data = {
			rmenu = require 'app.projectmgr.apps.recycles.rmenu'.get_branch_menu;
			id = arg.id;
		};
		kind = 'branch';
...
local function leaf_attr(arg)
	return {
		title = arg.name;
		data = {
			rmenu = require 'app.projectmgr.apps.recycles.rmenu'.get_leaf_menu;
			id = arg.id;
		};
		kind = 'leaf';
