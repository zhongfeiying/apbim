...
------------------------------------------------------------------------------------------
function Class:new(t)--必须有的 创建一个包含iup tree元素的类
	local t = t or {}
	setmetatable(t,self)
	self.__index = self;
	t.tree = iup.tree{}
...
function Class:map_attributes(attributes)
...
local function map_cb(self)
	self.Map = true --初始化Map变量为true
	init()
end 

function Class:map()
	self:get_element().map_cb = function (self) map_cb(self) end 
end
...
local function create_tree(t)
	...
	return iup.tree{
		...
		rastersize = t.rastersize or '200x200';
...
function Class:map_attributes(attributes,keys)
	for k,v in pairs(keys) do 
		local val = attributes[k]
		if val then 
			self.tree[k] = val
		end 
	end 
...
function Class:map()
	self.tree.map_cb = function (self) map_cb(self) end 
end
------------------------------------------------------------------------------
