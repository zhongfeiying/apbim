...
interface:
	pop(arg)
		Call example: require 'dlg_toolbar'.pop{menuData=,menuStyle=,styleFiles,on_ok,on_save}
...
local default_name_ = 'CURRENT_TOOLBAR_STYLE'
local loads_;
...
local function get_right_tree_data(id,data)
	...
	for i=1,get_node_count{tree =tree_right_ ,id = id} do
		...
		local temp_id = cur_id
		for i = 1,get_node_count{tree =tree_right_ ,id = cur_id} do 
			temp_id = temp_id + 1
			local temp_t = get_node_attribute{id = temp_id,tree = tree_right_} or {}
			if  not temp_t.name or temp_t.name == '' then 
				temp_t.name  = nil
			else
				temp_t.name = get_node_title{id = temp_id,tree = tree_right_}
			end
...
local function set_right_tree_data(tree,data,id)
	...
	for k,v in ipairs(data) do 
		local group = v.name or ( v[1]and v[1].name and string.match( v[1].name,'([^%.]+)'))  or 'Group' .. k
		if k == 1 then 
			add_branch_node{tree = tree_right_,id =cur_id,name =  group}
			cur_id = cur_id + 1 
		else 
			ins_branch_node{tree = tree_right_,id = cur_id,name = group}
			cur_id = cur_id + 1+ get_node_totalcount{id = cur_id,tree = tree_right_}
		end
		local temp_id  = cur_id
		for m,n in ipairs(v) do 
			...
			if m == 1 then 
				add_leaf_node{tree= tree_right_,id =temp_id,name = name }
				temp_id = temp_id + 1
			else 
				ins_leaf_node{tree= tree_right_,id = temp_id,name = name}
				temp_id = temp_id + 1
			end
			set_node_attribute{id = temp_id,attribute =n,tree = tree_right_}
		end
	end 
end
...
local function init_right_tree_data(arg)
	...
	if data then 
		...
		add_branch_node{tree =tree_right_,id = -1,name =  'ToolbarStyle'}
...
local function on_loading(arg,id,waitUpdate)
	...
	local posId ,tempTab= 0,{}
	local groupName = string.match(t.name,'([^%.]+)')
	local names_tab = get_title_index(tree_right_,posId)
	posId = names_tab and names_tab[groupName]  or posId
	if posId == 0 then 
		add_branch_node{tree = tree_right_,id =posId ,name = groupName}
		posId = posId+1
	end
	add_leaf_node{tree = tree_right_,id =posId ,name = t.name} 
	set_node_attribute{tree = tree_right_,id =posId+1,attribute = {name = t.name,image = t.image,keyword =t.keyword}}
...
local function action_tree()
	...
	local function init_tree_right_menu(id)
		...
		elseif get_node_depth{id = id,tree = tree_right_} == 1 then 
	...
	function tree_right_:dragdrop_cb(drag_id, drop_id, isshift, iscontrol)
		...
		elseif get_node_depth{tree = self,id = drop_id} == 1 then
			if get_node_states{tree = self,id =drop_id} == 'COLLAPSED' and get_node_kind{tree = self,id = drag_id} == 'leaf' then 
				return
			elseif  get_node_states{tree = self,id =drop_id} == 'EXPANDED' and get_node_kind{tree = self,id = drag_id} == 'branch' then 
				return 
			end
		elseif get_node_depth{tree = self,id = drop_id} == 2 then
			if get_node_depth{tree = self,id = drag_id} == 1 then 
				return 
			end
		end
